...
export type InvestmentReportId = "categories" | "ranking" | "feature-audit" | "by-theme";

interface InvestmentReportLayoutProps {
  activeReport: InvestmentReportId;
  title: string;
  description: string;
  eyebrow?: string;
  controls?: ReactNode;
...
const INVESTMENT_REPORTS: Record<InvestmentReportId, { href: string; label: string; detail: string }> = {
  categories: {
    href: "/analytics/investment/categories",
    label: "Categories",
    detail: "Feature, support, bugs, improvements, and overhead.",
  },
  ranking: {
    href: "/analytics/investment/ranking",
    label: "Feature ranking",
    detail: "The features consuming the most allocated capacity.",
  },
  "feature-audit": {
    href: "/analytics/investment/feature-audit",
    label: "Feature audit",
    detail: "Booked vs calculated investment with issue drilldown.",
  },
  "by-theme": {
    href: "/analytics/investment/by-theme",
    label: "Product",
    detail: "PMGT Product values as the portfolio lens.",
  },
};

const POPULATION_STEPS = [
  "Jira worklogs provide direct booked hours by issue, person, role, and month.",
  "PMGT feature and Product fields classify the work into portfolio slices.",
  "HRWorks availability and assignment rules allocate role overhead back into PMGT features and generic buckets.",
  "The analytics rebuild stores monthly allocation rows; page filters only reshape that prepared dataset.",
];

export function InvestmentReportLayout({
  activeReport,
  title,
  description,
  eyebrow = "Portfolio investment",
  controls,
  metrics,
  children,
}: InvestmentReportLayoutProps) {
  const section = JIRA_ANALYTICS_SECTIONS.find((item) => item.id === "portfolio");

  return (
    <div className="space-y-6">
      <section className="analytics-hero-panel p-6 md:p-8">
        <div className="analytics-hero-accent analytics-hero-accent--investment" aria-hidden />
        <div className="relative grid gap-8 xl:grid-cols-[minmax(0,1fr)_28rem]">
          ...
          <div className="analytics-hero-sidebar p-4">
            ...
            <nav className="space-y-2" aria-label="Investment reports">
              {(Object.keys(INVESTMENT_REPORTS) as InvestmentReportId[]).map((id) => {
                const report = INVESTMENT_REPORTS[id];
      ...
      <div className="min-w-0 space-y-6">
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_24rem]">
          <section className="analytics-section-panel rounded-[1.5rem] p-5 shadow-sm">
            ...
            <h3 className="mt-2 text-xl font-editorial font-bold text-on-surface">Allocation-ready data, not raw tables.</h3>
          ...
          <section className="elevated-panel rounded-[1.5rem] p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-on-surface-variant">
              Use categories for the portfolio mix, ranking for specific features, and Product for the roadmap ownership view.
            </p>
          </section>
        </div>
      </div>
